...
const UI_SET_EVBIT: libc::c_ulong = _ioc(1, b'U', 100, 4);
const UI_SET_KEYBIT: libc::c_ulong = _ioc(1, b'U', 101, 4);
const UI_DEV_SETUP: libc::c_ulong = _ioc(1, b'U', 3, UINPUT_SETUP_LEN);
const UI_DEV_CREATE: libc::c_ulong = _ioc(0, b'U', 1, 0);
const UI_DEV_DESTROY: libc::c_ulong = _ioc(0, b'U', 2, 0);
const UINPUT_SETUP_LEN: libc::c_ulong = 92; // input_id(8) + name[80] + ff_effects_max(4)

const fn _ioc(dir: u64, ty: u8, nr: u8, size: u64) -> libc::c_ulong {
    ((dir << 30) | (size << 16) | ((ty as u64) << 8) | nr as u64) as libc::c_ulong
}
...
impl Drop for Injector {
    fn drop(&mut self) {
        ...
        unsafe { libc::ioctl(self.dev.as_raw_fd(), UI_DEV_DESTROY) };
    }
}
...
impl Injector {
    ...
    fn create() -> Result<Self> {
        ...
        let set_bit = |req: libc::c_ulong, bit: u16| {
            // SAFETY: UI_SET_*BIT takes an int by value; fd is a live uinput device.
            unsafe { libc::ioctl(fd, req, bit as libc::c_int) }
        };
    ...
    fn query_event_node(&self) -> Result<String> {
        ...
        const UI_GET_SYSNAME: libc::c_ulong = _ioc(2, b'U', 44, 32);
