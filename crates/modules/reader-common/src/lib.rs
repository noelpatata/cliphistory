...
use cliphistory_proto::{HostToReader, ModuleManifest, ReaderToHost};
...
pub fn emit(frame: &ReaderToHost) -> Result<()> {
...
pub fn next_host_frame(reader: &mut impl BufRead) -> Option<HostToReader> {
    let mut line = String::new();
    match reader.read_line(&mut line) {
        Ok(0) | Err(_) => None,
        Ok(_) => match serde_json::from_str(line.trim()) {
            Ok(f) => Some(f),
            Err(_) => Some(HostToReader::Ping), // ignore garbage; keep alive
        },
    }
}
